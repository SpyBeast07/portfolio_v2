<script lang="ts">
	...
	import { cardStyle } from '$lib/components/admin/style';
	...
	function buildPayload(): Pick<SiteData, 'projectCategories' | 'projects'> {
		return {
	...
	const dirty = $derived(
		lastSaved !== null && JSON.stringify(buildPayload()) !== JSON.stringify(lastSaved)
	);

	const categoriesDirty = $derived(
		lastSaved !== null && !jsonEqual(categories, lastSaved.projectCategories)
	);
	...
	const projectSchema = $derived<AdminFieldDef[]>([
		{ key: 'title', label: 'Title' },
		{
			key: 'slug',
			label: 'Slug',
			hint: 'URL segment for the detail page — must be unique, e.g. "my-project".'
		},
		...
		{ key: 'highlights', label: 'Key highlights', type: 'tags' },
		{
			...
			type: 'textarea',
			rows: 4,
			hint: 'One short paragraph shown on the project detail page.'
		},
		{
			key: 'screenshots',
			label: 'Screenshots',
			type: 'tags',
			hint: 'Image paths or URLs shown in the Screenshots section.'
		}
	]);
...
<div>
	<EditorHeader
		title="Work"
		hint="Projects and the category filter. The Work / Blogs page headings are hard-coded in the codebase."
		{status}
		{dirty}
	...
	<div class="grid grid-cols-1 gap-6">
		<div class="rounded-xl p-6" style={cardStyle}>
			<AdminField
				...
				hint="Used by the category filter on the Work page. Keep 'All' as the first item."
			>
				<TagInput
					tags={categories}
